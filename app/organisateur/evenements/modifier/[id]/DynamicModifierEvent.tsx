...
export default function DynamicModifierEvent({
  event,
  games,
}: {
  event: Event;
  ...
  useEffect(() => {
    if (event) {
      ...
      setGameTitle(event.game.title);

      const dateObj = new Date(event.date);
      setDateDebut(dateObj.toISOString().split("T")[0]);
      setHeureDebut(
        `${String(dateObj.getHours()).padStart(2, "0")}:${String(dateObj.getMinutes()).padStart(2, "0")}`
      );

      const deadlineObj = new Date(event.inscriptionDeadline);
      setDateFin(deadlineObj.toISOString().split("T")[0]);
      setHeureFin(
        `${String(deadlineObj.getHours()).padStart(2, "0")}:${String(deadlineObj.getMinutes()).padStart(2, "0")}`
      );
    }
  }, [event]);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.layout}>
        ...
        <aside className={styles.sidebar}>
          ...
          <Button
            label={isLoading ? "Modification en cours..." : "Mettre à jour"}
            type="primary"
            fullWidth
